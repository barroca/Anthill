...
DCBuffer get_users_params(int argc, char *argv[]) {
	...
	if(argc != 8) {
		printf("HOW TO USE: %s <first collection> <first stream type> <mobius hosts file> <your 4 arguments...>", argv[0]);		exit(1);
	}
	...
	char *userArg7 = argv[7];
	work_buffer.pack("s", userArg7);
