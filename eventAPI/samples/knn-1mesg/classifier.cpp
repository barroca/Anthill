...
int classifier::trainning(AHEvent* event){
	...
	int numLines = ((int *)event->getData())[0];
	printf("NumLines = %d\n", numLines);

	char *buffPointer = (char *)event->getData();
	buffPointer += sizeof(int);
	for(int i = 0; i < numLines; i++){
		int class_id;
//		memcpy(&class_id, event->getData(), sizeof(int));
		memcpy(&class_id, buffPointer, sizeof(int));
		buffPointer += sizeof(int);

//		double* dimensions = get_dimensions((double*)&((int*)event->getData())[1],d);
		double* dimensions = get_dimensions((double*)buffPointer,d);

		buffPointer += sizeof(double) *d;
